...
export default function ChamberForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <Link to="/doctor/chambers" className="text-primary-600 text-sm font-medium mb-6 inline-block">← Back to chambers</Link>

      <h1 className="font-display text-2xl font-bold text-slate-800 mb-2">
        {isEdit ? 'Edit availability' : 'Add availability'}
      </h1>
      <p className="text-slate-600 mb-8">Select a facility and set where and when you work for each weekday. One facility per day; 4–8 hours per day.</p>
          ...
          <div className="flex flex-wrap gap-2">
            {DAYS.map((d) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition ${form.availableDays.includes(d.id) ? 'bg-primary-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                {d.name}
              </button>
            ))}
